...
- PR author が `app/renovate` または `renovate[bot]`、base branch が `main`、draft ではなく、`mergeable` が `MERGEABLE`。
...
- `cli/cli` の minor 更新は、release notes に security fix、telemetry、auth guidance、新しい command/subcommand、既存 command の bug fix が含まれていても、それだけでは禁止しない。以下をすべて満たす場合は自動マージしてよい。
  ...
  - repo 内検索で `gh` / `cli/cli` に依存する script、alias、workflow、設定が見つからない、または見つかっても今回の変更による影響なしと具体的に説明できる。
  - security fix を含む minor update で、上記の禁止条件に該当しない。
- repo 内検索で、変更対象が `aqua.yaml` の package pin、`renovate.json` の shared aqua preset 参照、Renovate config validate workflow、または README のローカル反映手順以外に強く結合していない。
...
- PR author が `app/renovate` または `renovate[bot]` ではない、または base branch が `main` ではない。
- draft、merge conflict あり、`mergeable` が `MERGEABLE` ではない。
- major update。
- `renovate.json` が `local>wim-web/renovate-config` を外す、`aquaproj/aqua-renovate-config:file` 以外の preset を追加する、対象 file pattern を変更する、automerge / schedule / base branch / labels などの Renovate policy を変える PR。
- `.github/workflows/renovate_config_validate.yaml` 以外の workflow、`install.sh`、fish 設定、git 設定、README、source code、migration、database、infra、deploy、Docker、Terraform 設定そのものを変更する PR。
